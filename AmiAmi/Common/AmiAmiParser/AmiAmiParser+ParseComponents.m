		...
		for (TFHppleElement *e in productInformationElementsArray) {
			for (int i = 0; i < [[e childrenWithTagName:@"dt"] count]; i++) {
				if ([[(TFHppleElement *)[e childrenWithTagName:@"dt"][i] text] isEqualToString:@"購入制限"] ||
                    ...
                    dictionaryInArray[@"Title"] = [(TFHppleElement *)[e childrenWithTagName:@"dt"][i] text];
                    dictionaryInArray[@"Content"] = [self mergeContentTexts:[e childrenWithTagName:@"dd"][i]];
...
+ (NSDictionary *)parshThumbnailWithTitle:(TFHppleElement *)element
{
	TFHppleElement *child = [element firstChild];
	return @{@"URL": element[@"href"], @"Thumbnail": child[@"src"], @"Title": child[@"alt"]};
}
