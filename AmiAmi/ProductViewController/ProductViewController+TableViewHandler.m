...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	if (indexPath.row == 0) {
		...
		[cell.currentProductImageView setImageWithURL:[NSURL URLWithString:self.productInfoDictionary[@"CurrentProduct"][@"Thumbnail"]] placeholderImage:[UIImage imageNamed:@"placeholder.png"] completed: ^(UIImage *image, NSError *error, SDImageCacheType cacheType) {
            if (error) NSLog(@"%@", error);
        }];
        
        cell.currentProductTitleTextView.text = self.productInfoDictionary[@"CurrentProduct"][@"Title"];
        ...
        cell.currentProductInformationTextView.text = productInformationString;
        ...
        cell.productTypeLabel.text = [self chooseProductTypeText:self.recordCellTypeArray[fixIndex]];
        
		cell.productsInfoArray = self.productInfoDictionary[self.recordCellTypeArray[fixIndex]];
		[cell.productCollectionView reloadData];
        
        @weakify(self);
        cell.onClickCollectionCell = ^(NSDictionary *result) {
            @strongify(self);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	if (indexPath.row == 0) {
		...
		if ([photos count]) {
			...
			UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"這個商品沒有圖片呦" message:nil delegate:nil cancelButtonTitle:@"確定" otherButtonTitles:nil];
			[alert show];
		}
	}
}
